    ...
    <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
      <el-tab-pane label="Updates Available" name="first">
        <div v-for="(plugin, index) in updatePlugins" :key="index" class="plugin needs-update">
          ...
          <el-button :icon="Upload">Update</el-button>
          <el-link class="plugin-relate-notes" :href="plugin.releaseNotes">Release Notes</el-link>
        </div>
        <el-button type="primary">Update All</el-button>
      </el-tab-pane>
      <el-tab-pane label="Updated Recently" name="second">
        <div v-for="(plugin, index) in installedPlugins" :key="index" class="plugin installed">
          <el-link :href="plugin.readme" class="plugin-name">{{ plugin.name }}</el-link>
          <div class="plugin-version"> {{ plugin.installedVersion }}</div>
          <el-link class="plugin-info" :href="plugin.readme">Docs</el-link>
        </div>   
      </el-tab-pane>
    </el-tabs>   
  </div>
